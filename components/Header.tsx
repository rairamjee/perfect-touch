  ...
  return (
    <header 
      className={`sticky top-0 z-50 bg-black transition-all duration-500 ${
        isScrolled ? 'shadow-2xl shadow-black/70 backdrop-blur-sm bg-black/95' : 'shadow-lg'
      }`}
    >
      ...
      <div className="bg-gradient-to-r from-gray-900 via-black to-gray-900 text-white py-3 px-4 border-b border-gray-800 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-yellow-400/5 to-transparent animate-shimmer-slow"></div>
          ...
          <div className="hidden md:flex items-center space-x-2">
            {[
              ...
              <Link
                key={item.id}
                href={item.id}
                className={`relative px-5 py-2.5 font-semibold transition-all duration-300 rounded-lg overflow-hidden group ${
                  isActive(item.id)
              ...
              >
                ...
                {isActive(item.id) && (
                  <>
                    <span className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-yellow-400 to-yellow-500"></span>
            ...
            <Link 
              href="/quote" 
              className="ml-4 relative bg-gradient-to-r from-yellow-400 to-yellow-500 text-black px-7 py-3 rounded-lg font-bold hover:from-yellow-300 hover:to-yellow-400 transition-all shadow-lg hover:shadow-yellow-500/50 hover:scale-110 transform overflow-hidden group"
            >
            ...
            <div className="flex flex-col space-y-2">
              {[
                ...
                <Link
                  key={item.id}
                  href={item.id}
                  className={`px-4 py-3 font-medium transition-all rounded-lg text-left flex items-center space-x-3 ${
                    isActive(item.id)
              ...
              <Link 
                href="/quote" 
                className="mt-3 bg-gradient-to-r from-yellow-400 to-yellow-500 text-black px-6 py-3 rounded-lg font-bold hover:from-yellow-300 hover:to-yellow-400 transition-all text-center shadow-lg flex items-center justify-center space-x-2"
                onClick={() => setIsMenuOpen(false)}
              >
                <span>Get Quote</span>
